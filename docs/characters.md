...
## 口癖・定型句の調整
1. 既存キャラクターを編集する場合は `characters/<id>/meta.yaml` 内の `phrases.idea_intro` を増減してください。`phrases.default` に書いた口癖リストは **LLM バックエンドが有効な場合のみ** 口調変換のプロンプトに渡されます（デフォルトで有効、無効にする場合は `--no-style-with-llm` を指定）。`phrases.idea_intro` に指定した定型句は、LLM を使わない場合に初回のみヒューリスティックで挿入されます。
