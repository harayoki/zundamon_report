...
## 口癖・定型句の調整
1. 既存キャラクターを編集する場合は `characters/<id>/meta.yaml` 内の `phrases.idea_intro` を増減してください。`phrases.default` に書いた口癖リストは **`--style-with-llm` オプションを付けて LLM バックエンドを有効にした場合のみ** 口調変換のプロンプトに渡されます。デフォルト（LLM 無効）では `phrases.idea_intro` に指定した定型句が初回のみヒューリスティックで挿入されます。
