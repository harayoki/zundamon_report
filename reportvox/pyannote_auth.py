...
def authenticate_only(ffmpeg_path: str) -> None:
    """pyannote/speaker-diarization の認証を試行し、結果を標準出力へ表示する。"""
    env_token = os.environ.get("PYANNOTE_TOKEN")
    token, _ = resolve_hf_token(env_token)
    env_info = EnvironmentInfo.collect(ffmpeg_path, env_token)

    if token is None:
        raise RuntimeError(
            append_env_details(
                "pyannote/speaker-diarization への認証には Hugging Face Token が必要です。\n"
                "環境変数 HF_TOKEN または PYANNOTE_TOKEN を設定してください。",
                env_info,
            )
        )
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    authenticate_only(args.ffmpeg_path)
